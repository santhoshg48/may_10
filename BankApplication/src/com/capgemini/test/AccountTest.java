...
public class AccountTest {
	...
	@Test
	public void whenValidAmountIsDeposited() throws InvalidAccountNumberException
	{
		Account account=new Account();
		account.setAccountNumber(101);
		account.setAmount(5000);
		when(accountRepository.searchAccount(101)).thenReturn(account);
		assertEquals(true, accountService.depositAmount(101, 5000));
	}
	
	@Test
	public void whenValidAmountIsWithDrawn() throws InvalidAccountNumberException
	{
		Account account=new Account();
		account.setAccountNumber(102);
		account.setAmount(5000);
		when(accountRepository.searchAccount(102)).thenReturn(account);
		assertEquals(true, accountService.withdrawAmount(102, 5000));
	}
	
	@Test(expected=com.capgemini.exceptions.InvalidAccountNumberException.class)
	public void whenInvalidAccountNumber() throws InvalidAccountNumberException
	{
